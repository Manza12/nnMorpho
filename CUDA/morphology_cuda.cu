...
__global__ void erosion_batched_forward_cuda_kernel(
		...
		const torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> strel_tensor,
		torch::PackedTensorAccessor32<float,3> output_tensor,
		torch::PackedTensorAccessor32<short,4> indexes) {
	...
	if (x < output_width && y < output_height) {
		for (int k = 0; k < batch_size; k++) {
			...
			output_tensor[k][y][x] = value;
			indexes[k][y][x][0] = index_i;
			indexes[k][y][x][1] = index_j;
		}
	}
}
...
__global__ void erosion_batched_backward_cuda_kernel(
		...
		const torch::PackedTensorAccessor32<short,4,torch::RestrictPtrTraits> indexes_accessor,
		torch::PackedTensorAccessor32<float,2> grad_input_accessor) {
	...
	if (x < grad_output_width && y < grad_output_height) {
		...
		short index_j;
		for (int k = 0; k < batch_size; k++) {
			index_i = indexes_accessor[k][y][x][0];
			index_j = indexes_accessor[k][y][x][1];
			atomicAdd(&grad_input_accessor[index_j][index_i], -grad_output_accessor[k][y][x]);
		}
	}
}
...
std::vector<torch::Tensor> erosion_batched_forward_cuda(
	...
	const auto output_width = input_width - strel_width + 1;
  	...
  	// Initialize indexes
  	auto options_indexes = torch::TensorOptions().device(input_tensor.device()).dtype(torch::kInt16);
  	torch::Tensor indexes = torch::zeros({batch_size, output_height, output_width, 2}, options_indexes);
	...
	auto strel_accessor = strel_tensor.packed_accessor32<float,2,torch::RestrictPtrTraits>();
	auto output_accessor = output_tensor.packed_accessor32<float,3>();
	auto indexes_accessor = indexes.packed_accessor32<short,4>();

	// Launch of the kernel
	erosion_batched_forward_cuda_kernel<<<grid_size, block_size>>>(input_accessor, strel_accessor, output_accessor, indexes_accessor);
...
torch::Tensor erosion_batched_backward_cuda(
	...
	short* strel_ptr = strel_shape.data_ptr<short>();
  	...
  	// Initialize output tensor
  	auto options = torch::TensorOptions().device(grad_output.device());
  	torch::Tensor grad_input = torch::zeros({strel_height, strel_width}, options);
	...
	auto indexes_accessor = indexes.packed_accessor32<short,4,torch::RestrictPtrTraits>();
	auto grad_input_accessor = grad_input.packed_accessor32<float,2>();

	// Launch of the kernel
	erosion_batched_backward_cuda_kernel<<<grid_size, block_size>>>(grad_output_accessor, indexes_accessor, grad_input_accessor);
...
std::vector<torch::Tensor> dilation_batched_forward_cuda(
	...
	// Launch of the kernel
	dilation_batched_forward_cuda_kernel<<<grid_size, block_size>>>(input_accessor, strel_accessor, output_accessor, indexes_accessor);
...
torch::Tensor dilation_batched_backward_cuda(
	...
	// Launch of the kernel
	dilation_batched_backward_cuda_kernel<<<grid_size, block_size>>>(grad_output_accessor, indexes_accessor, grad_input_accessor);
