...
def dilation(input_tensor: torch.Tensor,
    ...
    # Compute erosion
    result = dilation_cpp(input_tensor, structuring_element, origin[0], origin[1], block_shape[1], block_shape[0])
...
def opening(input_tensor: torch.Tensor,
    ...
    # Compute opening
    return dilation(erosion(input_tensor, structuring_element, origin, border, block_shape),
                    structuring_element, origin, block_shape)
...
def closing(input_tensor: torch.Tensor,
    ...
    """ Closing is one of the derived operations of Mathematical Morphology: it consists on dilating an image and then
        ...
        """
    # Compute opening
    return erosion(dilation(input_tensor, structuring_element, origin, block_shape),
                   structuring_element, origin, border, block_shape)
